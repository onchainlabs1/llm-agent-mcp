...
from agent.agent_core import AgentCore, AgentConfig, ToolCall, ToolResult
...
class TestAgentCore:
    ...
    @pytest.fixture
    def agent_core(self, mock_crm_service, mock_erp_service, temp_crm_mcp_schema, temp_erp_mcp_schema):
        """Create an AgentCore instance with mocked dependencies."""
        config = AgentConfig()
        
        with patch('agent.agent_core.CRMService', return_value=mock_crm_service), \
             patch('agent.agent_core.ERPService', return_value=mock_erp_service):
    ...
    def test_get_client_by_id_request(self, agent_core, mock_crm_service):
        ...
        assert response["result"]["name"] == "João Silva"
    ...
    def test_update_client_balance_request(self, agent_core, mock_crm_service):
        ...
        # Verify CRM service was called correctly
        mock_crm_service.update_client_balance.assert_called_once_with("joao123", 2500.0)
    ...
    def test_update_order_status_request(self, agent_core, mock_erp_service):
        ...
        assert response["parameters"] == {"order_id": "ORD-20241201-001", "new_status": "shipped"}
        assert "reasoning" in response
        assert "execution_time" in response
        
        # Verify ERP service was called correctly
        mock_erp_service.update_order_status.assert_called_once_with("ORD-20241201-001", "shipped")
